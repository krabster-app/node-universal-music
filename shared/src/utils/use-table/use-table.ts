...
export const useTableWithStore =
  ...
  <T extends Record<string, unknown>, PK extends SafeKeyTypes<T>>(
    ...
    return {
      ...
      updateBatch(externalItems: Record<PK, T[PK]>[]) {
        return externalItems.map(extElement =>
          extElement[pk] !== undefined
            ? { ...(this.get(extElement[pk]) || {}), ...extElement }
            : extElement,
        )
      },
      ...
      _import(externalSource) {
        ...
        options.onUpdate?.('_import')
      },
      _clean() {
        STORAGE[tableName] = {}
        options.onUpdate?.('_clean')
      },
      length() {
